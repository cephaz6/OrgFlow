...
- 2026-08-13 [Feature] packages/types: added shared domain contracts (organisation, user, membership, process definition/version, the Mongo definition document, condition AST, engine input/output, domain event envelope) per PRD.md §1-2, §4-7, §10; zero runtime deps.
...
- 2026-08-13 [Infrastructure] docker-compose.yml: added Postgres 16, MongoDB 7 and LocalStack 3 (s3, sqs, sns, events, scheduler) with healthchecks, matching the values already in local `.env`; wired `pnpm dev` to bring the stack up before `turbo run dev`; verified all three reach healthy and are reachable from the host on their mapped ports, then torn down cleanly.

- 2026-08-13 [Schema] packages/db/migrations: forward-only, four new migrations (`1786630667870_create-orgflow-app-role`, `1786630672128_identity-and-tenancy-tables`, `1786630677048_audit-and-idempotency`, `1786630683182_row-level-security`) creating the `orgflow_app` role, the PRD.md §2.1 identity/tenancy tables, `audit_events`/`idempotency_keys` from §2.5, and RLS per §2.6; role assumed via `SET LOCAL ROLE` per ADR-0009, not connected to directly; verified up and down against real Postgres.

- 2026-08-13 [Feature] packages/db: Kysely connection factory, the `withTenantTransaction` helper implementing `SET LOCAL ROLE orgflow_app` plus `SET LOCAL orgflow.organisation_id` (ADR-0004, ADR-0009), UUID v7 id generation (ADR-0003), and an `organisation_members` repository as the first tenant table exercising the mapping boundary to `@orgflow/types`; cross-tenant isolation verified by a real integration test against Postgres (`pnpm run test:integration`, excluded from the default CI-safe `test` run until Testcontainers wiring lands).
